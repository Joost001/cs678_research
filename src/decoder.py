...
X = torch.rand(batch_size, 7, embed_size)

X[0, 5:, :] = padding_indx
X[2, 4:, :] = padding_indx
Y = torch.randint(2, vocab_size, (batch_size, 10))
Y[0, 7:] = padding_indx
Y[1, 9:] = padding_indx
Y[2, 5:] = padding_indx

src_padding_mask, tgt_padding_mask, tgt_mask = create_mask(X, Y, padding_indx)
...
model_out = model(X, Y, src_padding_mask=src_padding_mask, tgt_padding_mask=tgt_padding_mask, tgt_mask=tgt_mask)
