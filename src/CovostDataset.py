...
from utils import load_df_from_tsv, get_fbank_wave_from_zip
...
class CovostDataset(Dataset):
    ...
    Note that args argument must be an argparse with the following args parsed:
    ...
    def __init__(self, tsv_root: str, zip_fbank_path: str, corpus_path: str, args):
        ...
        self.p = args
        self.max_n_frame = 0
        self.tokenizer = CovostTokenizer(corpus_root=self.corpus_path, src_lan=self.p.src_lan, tgt_lan=self.p.tgt_lan,
    ...
    def pad_data(self, data):
        ...
        for i in range(len(data)):
            ...
            while total_padding >= data[i]['fbank_wave'].shape[0]:
                tensor_fbank_wave = F.pad(tensor_fbank_wave, (0, 0, 0, data[i]['fbank_wave'].shape[0]-1),
